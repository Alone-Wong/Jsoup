 ...
 *  
 */
public class QYXFThread implements Runnable {
	private String url;
	private String url2;
	
	protected static Logger logger = Logger.getLogger(QYXFThread.class);
	
	static Properties id=PropertyUtils.loadProp("id");
	
	private static final String RULEID = id.getProperty("QYXFrule_id");
	
	private static final String TASK_ID = id.getProperty("QYXFtask_id");
	
	public QYXFThread(){};
	public QYXFThread(String url,String url2) {
		this.url =url;
		this.url2 = url2;
	}
	...
	private void CrawlerSender(JSONObject json,int num,String are){
		...
		for(int i = 0; i < num; i++){
			 ...
			 * 解析详细信息
			 */
			QYXFModel qyxf=new QYXFModel();
			
			//爬取属性并设置实体属性值
			if(parseDetailHtml(qyxf,json,i,num,are)==null);	
			
			//设置格式属性
			
			qyxf.setTaskid(TASK_ID);
			qyxf.setRuleid(RULEID);
			
			qyxf.setInserttime(inserttime);
			qyxf.setInsertdate(insertdate);
			qyxf.setId(id);
			
			qyxf.setUrl(qyxf.getStatdate()+qyxf.getAreacode()+qyxf.getValue()+"-"+url);
			
			
			/**
			 * 发送数据到ActiveMQ
			 */
			String JContent = JSON.toJSONString(qyxf);
		 ...
		 * @return dd
		 */
		private QYXFModel parseDetailHtml(QYXFModel qyxf,JSONObject json,int i,int num,String are){
			...
			//设置实体属性值
			setValueByreflect(qyxf,"statdate",sj);
			setValueByreflect(qyxf,"datefreq","年");
			setValueByreflect(qyxf,"country","中国");
			
			//根据获取的邮政编码，从配置表获取邮编名
			String reg=getCity(regCode);
			setValueByreflect(qyxf,"areacode",regCode);
			setValueByreflect(qyxf,"province",reg);
			if(regCode.equals("110000")||regCode.equals("120000")||regCode.equals("500000")||regCode.equals("310000")){
				
				setValueByreflect(qyxf,"city",reg);
			}else{
			setValueByreflect(qyxf,"city","");
			}
			
			setValueByreflect(qyxf,"district","");
			setValueByreflect(qyxf,"value",strdata);
		
			setValueByreflect(qyxf,"unit",unit);

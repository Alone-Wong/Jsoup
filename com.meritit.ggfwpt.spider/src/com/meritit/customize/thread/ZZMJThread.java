...
import com.meritit.customize.model.ZZMJModel;
/**
 * 住宅总建筑面积
 * @author viki
 *  
 */
public class ZZMJThread implements Runnable {
	private String url;
	private String url2;
	private String url3;
	private String url4;
	
	protected static Logger logger = Logger.getLogger(ZZMJThread.class);
	
	static Properties id=PropertyUtils.loadProp("id");
	
	private static final String RULEID = id.getProperty("ZZMJrule_id");
	
	private static final String TASK_ID = id.getProperty("ZZMJtask_id");
	
	public ZZMJThread(){};
	
	public ZZMJThread(String url,String url2,String url3,String url4) {
		this.url =url;
		this.url2 = url2;
		this.url3 = url3;
		this.url4 = url4;
	...
	public void run() {
		...
		initPageCrawler(url,url2,"qg");
		logger.info("正在抓取：省份数据");
		initPageCrawler(url3,url4,"sf");
	}
	
	/**
	 * 抓取所有信息
	 */
	public void initPageCrawler(String url,String url2,String are) {
		
		JSONObject json=CrawlerUtil.crawler(url);
		JSONObject json2=CrawlerUtil.crawler(url2);
		
		logger.info("获取到数据json串1："+json);
		logger.info("获取到数据json串2："+json2);
		...
		logger.info("开始解析数据...");
		CrawlerSender(json,json2,num,are);
	 ...
	 */
	private void CrawlerSender(JSONObject json,JSONObject json2,int num,String are){
		...
		for(int i = 0; i < num; i++){
			 ...
			 * 解析详细信息
			 */
			ZZMJModel zzmj=new ZZMJModel();
			
			//爬取属性并设置实体属性值
			if(parseDetailHtml(zzmj,json,json2,i,are)==null);	
			
			//设置格式属性
			
			zzmj.setTaskid(TASK_ID);
			zzmj.setRuleid(RULEID);
			
			zzmj.setInserttime(inserttime);
			zzmj.setInsertdate(insertdate);
			zzmj.setId(id);
			
			zzmj.setUrl(zzmj.getStatdate()+zzmj.getDim_jgfwmj()+zzmj.getDim_sgfwmj()+"-"+url);
			
			
			/**
			 * 发送数据到ActiveMQ
			 */
			String JContent = JSON.toJSONString(zzmj);
...
//			
		 ...
		 * @return dd
		 */
		private ZZMJModel parseDetailHtml(ZZMJModel zzmj,JSONObject json,JSONObject json2,int i,String are){
			...
			//dim_jgfwmj 房地产开发企业竣工房屋面积
			String strdata2 = json2.getJSONObject("returndata").getJSONArray("datanodes").getJSONObject(i).getJSONObject("data").getString("strdata");
			
			
			//设置实体属性值
			setValueByreflect(zzmj,"statdate",sj);
			setValueByreflect(zzmj,"datefreq","年");
			setValueByreflect(zzmj,"country","中国");
			
			//根据获取的邮政编码，从配置表获取邮编名
			String reg=getCity(regCode);
			setValueByreflect(zzmj,"areacode",regCode);
			setValueByreflect(zzmj,"province",reg);
			if(regCode.equals("110000")||regCode.equals("120000")||regCode.equals("500000")||regCode.equals("310000")){
				
				setValueByreflect(zzmj,"city",reg);
			}else{
			setValueByreflect(zzmj,"city","");
			}
			
			setValueByreflect(zzmj,"district","");
			setValueByreflect(zzmj,"dim_sgfwmj",strdata);
			setValueByreflect(zzmj,"dim_jgfwmj",strdata2);
		
			setValueByreflect(zzmj,"unit",unit);
